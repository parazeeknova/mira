...
|---|---|---|
| Google Cloud Vision (primary) | `GOOGLE_VISION_ENABLED=true` + ADC or `GOOGLE_APPLICATION_CREDENTIALS` | Raw image bytes, no hosting. 1000 free/mo |
...
**Google Cloud credentials (one-time, local dev):**

The Vision engine authenticates via Application Default Credentials (ADC) — no service account or IAM setup needed:
```bash
