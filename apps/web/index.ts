...
const publicDir = new URL("public/", import.meta.url),
  ...
  optionalString = (value: FormDataEntryValue | null): string | undefined => {
  ...
  buildIdentityId = (identities: { id: string }[], name: string): string => {
  ...
  handleEnrollmentRequest = async (req: Request): Promise<Response> => {
    ...
    if (
      req.method === "DELETE" &&
      url.pathname.startsWith("/api/enrollment/")
    ) {
      const identityId = decodeURIComponent(
        url.pathname.replace("/api/enrollment/", "")
      );
