...
describe("enrollment-store configuration", () => {
	test("isEnrollmentStoreConfigured returns false when env vars not set", () => {
		const originalBucket = Bun.env["MIRA_R2_BUCKET"],
		 originalEndpoint = Bun.env["MIRA_R2_ENDPOINT"],
		 originalKeyId = Bun.env["MIRA_R2_ACCESS_KEY_ID"],
		 originalSecretKey = Bun.env["MIRA_R2_SECRET_ACCESS_KEY"];

		delete Bun.env["MIRA_R2_BUCKET"];
		delete Bun.env["MIRA_R2_ENDPOINT"];
		delete Bun.env["MIRA_R2_ACCESS_KEY_ID"];
		delete Bun.env["MIRA_R2_SECRET_ACCESS_KEY"];

		const isConfigured = (): boolean => (
				Bun.env["MIRA_R2_BUCKET"] !== undefined &&
				Bun.env["MIRA_R2_ENDPOINT"] !== undefined &&
				Bun.env["MIRA_R2_ACCESS_KEY_ID"] !== undefined &&
				Bun.env["MIRA_R2_SECRET_ACCESS_KEY"] !== undefined
			);

		expect(isConfigured()).toBe(false);

		if (originalBucket !== undefined) {Bun.env["MIRA_R2_BUCKET"] = originalBucket;}
		if (originalEndpoint !== undefined) {Bun.env["MIRA_R2_ENDPOINT"] = originalEndpoint;}
		if (originalKeyId !== undefined) {Bun.env["MIRA_R2_ACCESS_KEY_ID"] = originalKeyId;}
		if (originalSecretKey !== undefined) {Bun.env["MIRA_R2_SECRET_ACCESS_KEY"] = originalSecretKey;}
	});
	...
	test("isEnrollmentStoreConfigured returns true when all env vars set", () => {
		const isConfigured = (): boolean => (
		 ...
		 originalBucket = Bun.env["MIRA_R2_BUCKET"],
		 originalEndpoint = Bun.env["MIRA_R2_ENDPOINT"],
		 originalKeyId = Bun.env["MIRA_R2_ACCESS_KEY_ID"],
		 originalSecretKey = Bun.env["MIRA_R2_SECRET_ACCESS_KEY"];
		...
		if (originalBucket === undefined) {delete Bun.env["MIRA_R2_BUCKET"];}
		else {Bun.env["MIRA_R2_BUCKET"] = originalBucket;}
		if (originalEndpoint === undefined) {delete Bun.env["MIRA_R2_ENDPOINT"];}
		else {Bun.env["MIRA_R2_ENDPOINT"] = originalEndpoint;}
		if (originalKeyId === undefined) {delete Bun.env["MIRA_R2_ACCESS_KEY_ID"];}
		else {Bun.env["MIRA_R2_ACCESS_KEY_ID"] = originalKeyId;}
		if (originalSecretKey === undefined) {delete Bun.env["MIRA_R2_SECRET_ACCESS_KEY"];}
		else {Bun.env["MIRA_R2_SECRET_ACCESS_KEY"] = originalSecretKey;}
	});
});
...
describe("identity file operations", () => {
	test("stale file detection logic", () => {
		const existing = { files: ["ref-1.jpg", "ref-2.jpg", "old-file.jpg"] },
		 newFiles = new Set(["ref-1.jpg", "ref-2.jpg"]),
		 staleFiles = existing.files.filter(
	...
	test("all files become stale when completely replaced", () => {
		const existing = { files: ["old-1.jpg", "old-2.jpg"] },
		 newFiles = new Set(["new-1.jpg", "new-2.jpg"]),
		 staleFiles = existing.files.filter(
			(filename) => !newFiles.has(filename)
		);
...
describe("identity lookup", () => {
	test("finds identity by id", () => {
		const identities = [
		 ...
		 found = identities.find((identity) => identity.id === "user-2");
	...
	test("returns undefined for non-existent id", () => {
		const identities = [
		 ...
		 found = identities.find((identity) => identity.id === "non-existent");
...
describe("identity filtering", () => {
	test("filters out identity by id", () => {
		const identities = [
		 ...
		 filtered = identities.filter((identity) => identity.id !== "user-2");
...
describe("identity update", () => {
	test("updates identity metadata while preserving files", () => {
		const identities = [
		 ...
		 updated = identities.map((identity) =>
			identity.id === "user-1"
				? {
						...identity,
