...
describe("enrollment-store configuration", () => {
  test("isEnrollmentStoreConfigured returns false when env vars not set", () => {
    ...
    if (originalBucket !== undefined) {
      Bun.env["MIRA_R2_BUCKET"] = originalBucket;
    }
    if (originalEndpoint !== undefined) {
      Bun.env["MIRA_R2_ENDPOINT"] = originalEndpoint;
    }
    if (originalKeyId !== undefined) {
      Bun.env["MIRA_R2_ACCESS_KEY_ID"] = originalKeyId;
    }
    if (originalSecretKey !== undefined) {
      Bun.env["MIRA_R2_SECRET_ACCESS_KEY"] = originalSecretKey;
    }
  });
...
describe("metadata normalization edge cases", () => {
  test("handles empty string values", async () => {
    ...
    const normalizeMetadata = (metadata: {
        ...
        ...(metadata.worksAt === undefined
          ? {}
          : { worksAt: metadata.worksAt }),
      }),
...
describe("identity file operations", () => {
  test("stale file detection logic", () => {
    const existing = { files: ["ref-1.jpg", "ref-2.jpg", "old-file.jpg"] },
      ...
      staleFiles = existing.files.filter((filename) => !newFiles.has(filename));
